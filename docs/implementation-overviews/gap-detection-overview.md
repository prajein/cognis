...
### Shared (`src/shared/`)
- **[NEW]** `events/createDomainEvent.ts` — small `DomainEvent<T>` factory with injectable clock + id source (testability + Arc-readiness). Lives in the skeleton-sanctioned, non-protected `shared/` area.
