...
**Status:** Approved for Implementation (with Refinements)
...
### 1.2 Two-Stage Translation (Decoupling DOM from Domain)
...
```
If the host platform's DOM changes, only the `Translator` logic and selector mappings need updating. The rest of the pipeline remains completely insulated.

### 1.3 Explicit Failure Policy
...
- Retry discovery (e.g., wait for SPA navigation to restore the DOM).
...
- **Tasks:**
  ...
  - Implement a `MutationObserver` yielding raw DOM snapshots.
  - Translate snapshots into `response.started`, `response.chunk` (deltas only), and `response.completed`.
