...
export class QualityAnalyzer implements ResponseAnalyzer {
    ...
    public analyze(
        ...
        // A real quality analyzer would likely compose the results of
        // the other analyzers. For efficiency, the orchestrator/pipeline
        // usually runs them all.
        //
        // Here we can assume the pipeline already ran them and just
        // aggregate, OR we can run them ourselves if this is standalone.
        //
        ...
        const reasoningResult =
            this.reasoningAnalyzer.analyze(
                responseText,
                promptHash
            );

        const completenessResult =
            this.completenessAnalyzer.analyze(
                responseText,
                promptHash
            );

        const assumptionResult =
            this.assumptionAnalyzer.analyze(
                responseText,
                promptHash
            );

        const gapResult =
            this.gapCompletionAnalyzer.analyze(
                responseText,
                promptHash
            );
        ...
        // Gap          (15%)
        //
        // Assumption density is inverted because a higher assumption
        // density means worse quality.
        let score =
